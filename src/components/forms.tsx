...
export function PasswordForm() {
 ...
 return <form className="form" onSubmit={submit}><label>Senha atual<input name="currentPassword" type="password" autoComplete="current-password" required maxLength={128}/></label><label>Nova senha<input aria-label="Nova senha" aria-describedby="new-password-help" name="password" type="password" autoComplete="new-password" required minLength={8} maxLength={128}/><small id="new-password-help">Mínimo de 8 caracteres, contendo letra maiúscula, minúscula, número e caractere especial.</small></label><label>Confirmar nova senha<input name="confirmation" type="password" autoComplete="new-password" required minLength={8} maxLength={128}/></label>{error&&<p className="error" role="alert">{error}</p>}<button disabled={busy}>{busy?'Salvando…':'Salvar nova senha'}</button></form>;
}
